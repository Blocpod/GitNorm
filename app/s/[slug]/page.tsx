...
async function publicProject(slug:string){
  await ensureSchema();
  const project=await getD1().prepare(`SELECT p.id,p.slug,p.title,p.description,p.about,p.icon,p.accent,p.updated_at AS updatedAt,pr.display_name AS creator,pr.handle,v.id AS versionId,v.number,v.file_count AS fileCount,v.total_size AS totalSize FROM projects p JOIN profiles pr ON pr.id=p.owner_id JOIN versions v ON v.project_id=p.id AND v.number=(SELECT MAX(number) FROM versions WHERE project_id=p.id) WHERE p.slug=? AND p.visibility='public' AND p.deleted_at IS NULL`).bind(slug).first<{id:string;slug:string;title:string;description:string;about:string;icon:string;accent:string;updatedAt:number;creator:string;handle:string;versionId:string;number:number;fileCount:number;totalSize:number}>();
  if(!project)return null;
  const files=await getD1().prepare('SELECT id,path,mime_type AS mimeType,size FROM project_files WHERE version_id=? ORDER BY path').bind(project.versionId).all<{id:string;path:string;mimeType:string;size:number}>();
  return {project,files:files.results};
}

export async function generateMetadata({params}:{params:Promise<{slug:string}>}):Promise<Metadata>{
  const {slug}=await params; const data=await publicProject(slug); if(!data)return {title:'Project not found — GitNorm'};
  const {project}=data; return {title:`${project.title} — GitNorm`,description:project.description||`A project by ${project.creator}`,openGraph:{title:project.title,description:project.description||`A project by ${project.creator}`,images:[]},twitter:{card:'summary',title:project.title,description:project.description||`A project by ${project.creator}`,images:[]}};
}

export default async function SharedProject({params}:{params:Promise<{slug:string}>}){
  const {slug}=await params; const data=await publicProject(slug); if(!data)notFound(); const {project,files}=data;
...
function fileIcon(path:string){if(/\.(png|jpg|jpeg|gif|webp)$/i.test(path))return '▧';if(/\.(md|txt)$/i.test(path))return '≡';if(/\.(html|css|js|ts|tsx|jsx|json)$/i.test(path))return '⌘';return '·';}
